...
{
public:
	EPSLine(cv::Point2f p_start, cv::Point2f p_end, double pix_to_mm_scale) : p_start(p_start),
																			  p_end(p_end),
...
{
public:
	EPSPolygon(const std::vector<cv::Point2f> &points, double pix_to_mm_scale) : points(points),
																				 pix_to_mm_scale(pix_to_mm_scale)
	{
	}
...
{
public:
	EPSText(const cv::Point2f &pos, const std::string &text, double rotation_rad, double pix_to_mm_scale) : pos(pos),
																											text(text),
...
{
public:
	EPSBezier(const std::vector<BezierCurve> &curve, double pix_to_mm_scale) : curve(curve),
																			   pix_to_mm_scale(pix_to_mm_scale)
	{
	}
...
{
public:
	EPSCircle(cv::Point2f center, double radius, double pix_to_mm_scale) : center(center),
																		   radius(radius),
																		   pix_to_mm_scale(pix_to_mm_scale)
	{
	}
...
{
public:
	EPSSaver(const Texture &texture, const std::string &output_name = "") : VectorGraphicsSaver(texture, output_name),
																			pixel_to_mm_scale(25.4 / texture.dpi)
	{
		...
		add_polygon({cv::Point2f(0.0f, 0.0f),
	...
	EPSSaver(const std::string &output_name) : VectorGraphicsSaver(output_name),
											   pixel_to_mm_scale(-1),
