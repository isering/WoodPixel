...
public:
  ...
  mat(int height, int width, const T& init = T()) :
    m_rotate_x(0),
    m_rotate_y(0)
  {
  ...
  {
    m_rotate_x = rotate_x;
    while (m_rotate_x < 0) m_rotate_x += m_width;
    while (m_rotate_x >= m_width) m_rotate_x -= m_width;
  }

  void set_rotate_y(int rotate_y)
  {
    m_rotate_y = rotate_y;
    while (m_rotate_y < 0) m_rotate_y += m_height;
    while (m_rotate_y >= m_height) m_rotate_y -= m_height;
  }
...
private:
  ...
  {
    index_x += rotate_x;
    while (index_x < 0) index_x += m_width;
    while (index_x >= m_width) index_x -= m_width;
    return index_x;
  }
  ...
  {
    index_y += rotate_y;
    while (index_y < 0) index_y += m_height;
    while (index_y >= m_height) index_y -= m_height;
    return index_y;
  }
